...
def connect_to_server(server_name):
    ...
    # Obtengo la ip del servidor al que me quiero conectar
    ip_address = socket.gethostbyname(server_name)
    # Aviso al usuario que voy a realizar un intento de conexión
    sys.stderr.write("Contactando al servidor en %s...\n" % ip_address)
    """
    Creo un socket con los parametros:
    * La constante AF_INET incluida en la libreria socket, que
    me permite indicarle al socket el tipo de direcciónes con las
    que me voy a comunicar, IPV4.
    * La constante SOCK_STREAM indicá que el socket estara dedicado
    a una conexión TCP
    """
    socket_conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    """
    Conecta el nuestro socket con el socket del servidor 
    en el puerto 'HTTP_PORT' (por convención, el num 80), en la
    dirección 'ip_address'.
    """
    socket_conn.connect((ip_address, HTTP_PORT))
